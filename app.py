...
def draw_roll(D, L):
    ...
    R = D / 2
    theta = (360 * L) / (np.pi * D)  # angolo corretto in gradi
    ...
    # Punto dove la colla deve finire dopo la rotazione
    angle_colla_rotated = angle_colla - theta_rad
    x_target = R * np.cos(angle_colla_rotated)
    y_target = R * np.sin(angle_colla_rotated)

    # Visualizzazione del tratto di velo
    ...
    ax.plot([x_target], [y_target], 'o', color="#1e88e5", markersize=8, label='Colla dopo rotazione')
    ...
    angle_label = (angle_colla + angle_colla_rotated) / 2
    ...
    ax.set_aspect('equal')
    ax.set_title(f"\u2728 Bobina Interattiva \u2728\nDiametro = {D:.0f} mm | Lunghezza Velo = {L:.0f} mm | Rotazione θ = {theta:.2f}°",
